...
import { Box, Plus, Minus, Trash2, Calendar as CalendarIcon, Check, AlertTriangle } from 'lucide-react';
...
const CubeManager: React.FC<CubeManagerProps> = ({ cubes, onAddCube, onDeleteCube, onUpdateQuantity }) => {
  ...
  const [quantity, setQuantity] = useState(10); // 기본 수량 10개로 설정
  // 기본 선택 색상을 PRESET_COLORS의 첫 번째(index 0)로 변경
  const [selectedColor, setSelectedColor] = useState(PRESET_COLORS[0].hex);

  // 제조일을 변경하면 기본 만료일을 2주 뒤로 세팅해주되, 사용자가 수동으로 바꿀 수 있음
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setName('');
    setQuantity(10); // 추가 후 다시 10개로 초기화
  };
  ...
  return (
    <div className="bg-white rounded-3xl shadow-xl p-6 border border-slate-100">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-4">
            ...
            <div className="flex gap-2">
              ...
              <div className="flex-1">
                <label className="text-[10px] font-bold text-slate-400 uppercase ml-1 mb-1 block text-pink-500">만료일</label>
                <input
                  type="date"
                  value={expiryDate}
          ...
          <div className="flex flex-col justify-between space-y-4">
            <div>
              <label className="text-[10px] font-bold text-slate-400 uppercase ml-1 mb-1 block">보관 수량 (기본 10개)</label>
              <div className="flex items-center justify-between bg-slate-50 border border-slate-200 rounded-2xl p-2 h-[52px]">
                <button 
                  type="button" 
                  onClick={() => setQuantity(prev => Math.max(1, prev - 1))}
                  className="w-10 h-10 flex items-center justify-center text-slate-400 hover:text-pink-500 hover:bg-white rounded-xl transition-all"
                >
                  <Minus className="w-5 h-5" />
                </button>
                <div className="flex items-center gap-1">
                  <span className="text-xl font-black text-slate-700">{quantity}</span>
      ...
      <div className="space-y-3 max-h-[500px] overflow-y-auto custom-scrollbar pr-1 mt-8 pt-6 border-t border-slate-50">
        <div className="flex items-center justify-between mb-2 px-1">
          <h3 className="text-xs font-bold text-slate-400 flex items-center gap-1">
            <Box className="w-3 h-3" /> 보관 리스트
          </h3>
          <span className="text-[10px] text-slate-300">최근 등록순</span>
        </div>
        ...
        {cubes.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-slate-300">
            ...
            <p className="text-[10px] mt-1">위 양식을 작성해 큐브를 추가해보세요!</p>
          </div>
        ) : (
          cubes.map(cube => {
            const daysLeft = differenceInDays(parseISO(cube.expiryDate), new Date());
            ...
            return (
              <div 
                key={cube.id} 
                className={`group p-4 rounded-3xl border-l-[12px] flex items-center justify-between transition-all bg-white shadow-sm border border-slate-100 hover:shadow-md ${isExpired ? 'opacity-60' : ''}`}
              ...
              >
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    >
                      {isExpired ? '만료됨' : isNearExpiry ? <><AlertTriangle className="w-3 h-3"/> D-{daysLeft}</> : `D-${daysLeft}`}
                    </span>
                  </div>
                  <div className="flex items-center gap-4 mt-1.5">
                    <div className="flex items-center gap-1 text-[10px] text-slate-400">
                      <CalendarIcon className="w-3 h-3" />
                      <span>{cube.madeDate} 조리</span>
                    </div>
                    <div className="flex items-center gap-1 text-[10px] text-pink-400 font-bold">
                      <Check className="w-3 h-3" />
                      <span>{cube.expiryDate} 만료</span>
                    </div>
                  </div>
                </div>
